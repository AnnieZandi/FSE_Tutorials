...
def test_only_expenses(expenses_only_transactions):
    ...
    assert calculate_balance(expenses_only_transactions) == Decimal('-1000.00') # Since we have no income, we are overspending
...
def test_only_income(income_only_transactions):
    ...
    assert calculate_balance(income_only_transactions) == Decimal('3500.00') # Since we have no expenses, we are saving well
    assert check_financial_health(income_only_transactions) == "Saving well"  # Since expenses is 0, we cannot determine financial health

#TODO Complete this test
def test_mixed_transactions(sample_transactions):
    assert calculate_total_expenses(sample_transactions) == Decimal('-4750.50')
    assert calculate_total_income(sample_transactions) == Decimal('11500.00')
    assert calculate_balance(sample_transactions) == Decimal('6749.50') # Since we have more income than expenses, we are saving well
